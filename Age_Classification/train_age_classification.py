...
def  main(args):
    ...
    if args.model != None:
        ...
        model.fit_generator(train_generator, epochs=NUM_EPOCHS, workers=8, steps_per_epoch=num_train_images // BATCH_SIZE,  shuffle=True, callbacks=callbacks_list)
    else :
        ...
        csv_logger = CSVLogger('log/log.csv', append=False, separator=';')
        callbacks_list = [checkpoint,csv_logger]
        finetune_model.fit_generator(train_generator, epochs=NUM_EPOCHS, workers=8, steps_per_epoch=num_train_images // BATCH_SIZE, shuffle=True, callbacks=callbacks_list)
